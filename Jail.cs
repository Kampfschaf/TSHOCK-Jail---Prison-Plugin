...
namespace JailPrison
{
    ...
    public class Jail : TerrariaPlugin
    {
        ...
        {
            get { return "DarkunderdoG, PlayKampfschaf"; }
        }
        ...
        public static void SetupConfig()
        {
            ...
            catch (Exception ex)
            {
                ...
                Console.WriteLine("Jail Config Exception");
                        ...
                        if (player != null && player.TSPlayer != null)
                        {
                            ...
                            if (player.prisonMode && JPConfig.prisonmode)
                            {
                                if (!currentregionlist.Contains("prison"))
                                {
                                    ...
                                    if (warp != null)
                                    {
                                        if (player.TSPlayer.Teleport((int)warp.WarpPos.X * 16, (int)warp.WarpPos.Y * 16))
                                        {
                                            if (player.prisonModeSpam)
                                            {
                                                player.TSPlayer.SendMessage("You are stuck in prison - An Admin or Mod Will Need To Let You Out...", Color.Aqua);
                                ...
                                if (!currentregionlist.Contains("jail"))
                                {
                                    ...
                                    if (warp != null)
                                    {
                                        if (player.TSPlayer.Teleport((int)warp.WarpPos.X * 16, (int)(warp.WarpPos.Y) * 16))
                                        {
                                            ...
                                            {
                                                player.TSPlayer.SendMessage("You Cannot Get Out Of Jail Without Reading The Rules", Color.Aqua);
                            ...
                            if (!player.chestMode && JPConfig.chestzones)
                            {
                                ...
                                else
                                {
                                    ...
                                    player.TSPlayer.SendMessage("Your Allowed Time In The Chest Room Is Now Up!", Color.Aqua);
                                ...
                                if (region.Contains("chests"))
                                {
                                    if (!region.Contains(player.TSPlayer.Name.ToLower()) && !region.Contains(player.TSPlayer.Group.Name.ToLower()) && !region.Contains(prefix.ToLower()))
                                    {
                                        if (player.TSPlayer.Teleport(Main.spawnTileX * 16, Main.spawnTileY + 3 * 16))
                                        {
                                            ...
                                            {
                                                player.TSPlayer.SendMessage("You Do Not Have Permission To Enter This Chest Room", Color.Aqua);
                            ...
                            if (!player.killMode && !player.TSPlayer.Group.HasPermission("killzone") && JPConfig.killzones)
                            {
                                ...
                                if (region.Contains("kill"))
                                {
                                    if (!region.Contains(player.TSPlayer.Name.ToLower()) && !region.Contains(player.TSPlayer.Group.Name.ToLower()) && !region.Contains(prefix.ToLower()))
                                    {
                                        ...
                                        {
                                            player.TSPlayer.SendMessage("You Entered The " + region + " ZonE", Color.Aqua);
                                            TShock.Utils.Broadcast(player.TSPlayer.Name + " Was Killed Because They Entered The " + region + " ZonE!", Color.Aqua);
                            ...
                            if (player.lasttileX != 0 && player.lasttileY != 0 && !currentregionlist.Contains("afk") && JPConfig.afkmode)
                            {
                                ...
                                else if (player.idle > JPConfig.afktime && !player.prisonMode)
                                {
                                    ...
                                    if (player.TSPlayer.Teleport((int)warp.WarpPos.X * 16, (int)(warp.WarpPos.Y + 3) * 16))
                                    {
                                        player.TSPlayer.SendMessage("You Have Been Warped To The AFK Zone. Use the /back command to go back!", Color.Aqua);
                                        TShock.Utils.Broadcast(player.TSPlayer.Name + " Is Away From His/Her Keyboard And Has Been Warped To The AFK Zone!", Color.Aqua);
                                    }
                                }
                            }
                        }
        ...
        public void OnChat(ServerChatEventArgs e)
        {
            ...
            if (cmd == "/warp")
            {
                ...
                {
                    if (TShock.Warps.FindWarp(text.Split(' ')[1]) != null)
                    {
                        ...
                        {
                            tsplr.SendMessage("You are stuck in prison - An Admin or Mod Will Need To Let You Out...", Color.Aqua);
                            e.Handled = true;
                            return;
                        }
                    }
            ...
            else if (cmd == "/home")
            {
                ...
                tsplr.Spawn();
                tsplr.SendMessage("Teleported to your spawnpoint.", Color.Aqua);
                e.Handled = true;
                return;
            }
            ...
            else if (cmd == "/spawn")
            {
                ...
                else
                {
                    if (tsplr.Teleport(Main.spawnTileX * 16, (Main.spawnTileY * 16) - 48))
                        tsplr.SendMessage("Teleported to the map's spawnpoint.", Color.Aqua);
                    e.Handled = true;
                    return;
                }
            }
            else if ((cmd == "/afk" || cmd == "/i" || cmd == "/give") && JPConfig.cmdlimit)
            {
                if (text.Split(' ').Length > 1)
                {
                    ...
                    {
                        tsplr.SendMessage("You Can Only Use An Item Command Every " + JPConfig.cmdtime + " Seconds You Have " + jpPly.itemAmount + " Seconds Left", Color.Aqua);
                        e.Handled = true;
                        return;
                    }
            ...
            else if (cmd == "/heal" && JPConfig.cmdlimit)
            {
                ...
                {
                    tsplr.SendMessage("You Can Only Use The Heal Command Every " + JPConfig.cmdtime + " Seconds You Have " + jpPly.healAmount + " Seconds Left", Color.Aqua);
                    e.Handled = true;
                    return;
                }
        ...
        private void warpjail(CommandArgs args)
        {
            ...
            {
                args.Player.SendMessage("You cannot use teleport commands!", Color.Aqua);
                return;
            }
            ...
            if (jpPly.prisonMode)
            {
                args.Player.SendMessage("You are stuck in prison - An Admin or Mod Will Need To Let You Out...", Color.Aqua);
                return;
            }
        ...
        private void imprison(CommandArgs args)
        {
            ...
            if (jpPly.prisonMode)
            {
                args.Player.SendMessage("Player Is Already In Prison", Color.Aqua);
                return;
            }
            string warpName = "prison";
            var warp = TShock.Warps.Find(warpName);
        ...
        private void setfree(CommandArgs args)
        {
            ...
            if (!jpPly.prisonMode)
            {
                args.Player.SendMessage("Player Is Already Free", Color.Aqua);
                return;
            }
        ...
        private void jailreload(CommandArgs args)
        {
            ...
            args.Player.SendMessage("Jail Reload Initiated", Color.Aqua);
        }
        ...
        private void chests(CommandArgs args)
        {
            ...
            if (jpPly.chestMode)
            {
                ...
                foundplr[0].SendMessage("You Have Been Granted Access To The Chests By " + args.Player.Name + " For " + jpPly.chestAmount + " Seconds!", Color.Aqua);
                args.Player.SendMessage("You Have Granted Access For The Chests To " + foundplr[0].Name + " For " + jpPly.chestAmount + " Seconds!", Color.Aqua);
                return;
            }
            if (!jpPly.chestMode)
            {
                jpPly.chestMode = !jpPly.chestMode;
                foundplr[0].SendMessage("Your Access To The Chests Has Been Revoked By " + args.Player.Name, Color.Aqua);
                args.Player.SendMessage("You Have Revoked Access From The Chests From " + foundplr[0].Name, Color.Aqua);
        ...
        {
            var jpPly = Players[args.Player.Index];
            TShock.Players[args.Player.Index].SendMessage("You Have Been AFK For: " + jpPly.idle, Color.Aqua);
        }
        ...
        private void sendafk(CommandArgs args)
        {
            ...
            if (args.Player.Teleport((int)warp.WarpPos.X * 16, (int)warp.WarpPos.Y * 16 + 3))
            {
                args.Player.SendMessage("You Have Been Warped To The AFK Zone. Use the /back command to go back!", Color.Aqua);
                TShock.Utils.Broadcast(args.Player.Name + " Is Away From His/Her Keyboard And Has Been Warped To The AFK Zone!", Color.Aqua);
            }
        }
        ...
        private void sendback(CommandArgs args)
        {
            ...
            if (jpPly.backtileX != 0)
            {
                ...
                {
                    args.Player.SendMessage("You Have Been Warped Back To Where You Were", Color.Aqua);
                    TShock.Utils.Broadcast(args.Player.Name + " Is Back From AFK! YAY!!!", Color.Aqua);
                    jpPly.backtileX = 0;
                    jpPly.backtileY = 0;
                }
            }
            else
                args.Player.SendMessage("Unable To Send You Back From AFK", Color.Aqua);
        }
        ...
        private static void setidletime(CommandArgs args)
        {
            ...
            args.Player.SendMessage("Idle Time Set To: " + JPConfig.afktime, Color.Aqua);
        }
        ...
        private static void setcmdtime(CommandArgs args)
        {
            ...
            args.Player.SendMessage("Command Time Set To: " + JPConfig.cmdtime, Color.Aqua);
        }
        ...
        private void killzone(CommandArgs args)
        {
            ...
            if (!jpPly.killMode)
            {
                ...
                foundplr[0].SendMessage("You Have Been Granted Access To Kill Zones From " + args.Player.Name, Color.Aqua);
                args.Player.SendMessage("You Have Granted Access To Kill Zones For " + foundplr[0].Name, Color.Aqua);
                return;
            }
            if (jpPly.killMode)
            {
                jpPly.killMode = !jpPly.killMode;
                foundplr[0].SendMessage("Your Access To The Kill Zones Has Been Revoked By " + args.Player.Name, Color.Aqua);
                args.Player.SendMessage("You Have Revoked Access From Kill Zones For " + foundplr[0].Name, Color.Aqua);
                return;
            }
        }
